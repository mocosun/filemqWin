...
struct _fmq_patch_t {
    ...
    char *virtual2;              //  Virtual file name
...
fmq_patch_new (char *path, fmq_file_t *file, fmq_patch_op_t op, char *alias)
{
    ...
    assert (*filename != '/');
    self->virtual2 = (char*)malloc (strlen (alias) + strlen (filename) + 2);
    if (alias [strlen (alias) - 1] == '/')
        sprintf (self->virtual2, "%s%s", alias, filename);
    else
        sprintf (self->virtual2, "%s/%s", alias, filename);
...
fmq_patch_dup (fmq_patch_t *self)
{
    ...
    copy->op = self->op;
    copy->virtual2 = strdup (self->virtual2);
