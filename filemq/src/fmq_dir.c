...
fmq_dir_diff (fmq_dir_t *older, fmq_dir_t *newer, char *alias)
{
    ...
    while (old_files [old_index] || new_files [new_index]) {
        ...
        fmq_file_t *new2 = new_files [new_index];
        ...
        if (!new2)
            ...
            cmp = strcmp (fmq_file_name (old, NULL), fmq_file_name (new2, NULL));

        if (cmp > 0) {
            //  New file was created
            if (fmq_file_stable (new2))
                zlist_append (patches, fmq_patch_new (newer->path, new2, patch_create, alias));
            old_index--;
        }
        else
        if (cmp < 0) {
        ...
        if (cmp == 0 && fmq_file_stable (new2)) {
            if (fmq_file_stable (old)) {
                ...
                if (fmq_file_time (new2) != fmq_file_time (old)
                ||  fmq_file_size (new2) != fmq_file_size (old))
                    zlist_append (patches, fmq_patch_new (newer->path, new2, patch_create, alias));
            }
            else
